...
class GoogleNews:
    def __init__(self, lang = 'en', country = 'US', timeout = 5):
    ...
    def __parse_feed(self, feed_url, proxies=None, scraping_bee = None):
        ...
        if proxies:
            r = requests.get(feed_url, proxies = proxies, timeout=self.timeout)
        else:
            r = requests.get(feed_url, timeout=self.timeout)
